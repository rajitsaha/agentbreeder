...
The APS sidecar handles everything the platform needs to do: RAG retrieval, memory persistence, tool execution, A2A proxying, OpenTelemetry tracing, and cost recording. Your agent code calls the sidecar over a local HTTP connection on port 9001. **You write zero infrastructure code.**
