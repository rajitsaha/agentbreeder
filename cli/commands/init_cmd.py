...
# ─── Go framework metadata ──────────────────────────────────────────
# Tier-2 polyglot — Track I (#165). Today only `custom` is shipped; eino,
# genkit, dapr_agents, langchaingo, anthropic_go_sdk are tracked in
# follow-ups.
...
GO_FRAMEWORKS = [
    {
        ...
        "name": "Custom",
        "icon": "⚙️",
        "tagline": "Bring your own Go agent code (uses the AgentBreeder Go SDK)",
        "model": "claude-sonnet-4-20250514",
    },
]
...
# ─── Go scaffolding (Tier-2 polyglot, Track I phase 1) ──────────────


def _go_agent_yaml(name: str, framework: str, cloud: str, team: str, owner: str) -> str:
    return f"""\
# AgentBreeder configuration — generated by `agentbreeder init --lang go`
# Docs: https://github.com/agentbreeder/agentbreeder

name: {name}
version: 0.1.0
description: "{name} — Go agent powered by the AgentBreeder Go SDK"
team: {team}
owner: {owner}
tags:
  - go
...
def _go_main_go(name: str) -> str:
    """Minimal main.go — uses the Go SDK and echoes input.

    Mirrors examples/go-agent/main.go but stripped to its bones so the
    scaffold compiles without an API key. Users add Anthropic/OpenAI calls
    as a second step.
    """
    return f"""\
// {name} — generated by `agentbreeder init --lang go`.
// Replace the body of `invoke` with your agent logic.
package main
...
\tlogger.Info("starting agent", "addr", addr)
...
def _go_main_test_go(name: str) -> str:
    _ = name  # reserved for future use; keeps signature consistent with siblings
    return """\
package main
...
func TestInvoke_EchoesString(t *testing.T) {{
\tt.Parallel()
\tvar resp agentbreeder.InvokeResponse
\tif err := invoke(context.Background(), agentbreeder.InvokeRequest{{Input: agentbreeder.NewStringInput("hi")}}, &resp); err != nil {{
...
def _go_mod(name: str) -> str:
    """go.mod for a freshly scaffolded Go agent.

    Pins the Go SDK at v0.1.0; users can `go get -u` to bump.
    """
...
require github.com/agentbreeder/agentbreeder/sdk/go/agentbreeder v0.1.0
"""
...
def _go_env_example() -> str:
    return """\
# Environment variables for your Go agent
AGENTBREEDER_ENV=development

# Required when calling Anthropic Claude:
ANTHROPIC_API_KEY=sk-ant-...

# Optional — bearer token enforced on /invoke and /stream:
# AGENT_AUTH_TOKEN=replace-me
"""


def _go_readme(name: str, framework: str, cloud: str) -> str:
    return f"""\
# {name}

⚙️ Go agent — scaffolded by AgentBreeder (Track I, polyglot Tier-2).

Uses the AgentBreeder [Go SDK](https://github.com/agentbreeder/agentbreeder/tree/main/sdk/go/agentbreeder)
to satisfy Runtime Contract v1 (`/health`, `/invoke`, `/stream`,
`/openapi.json`).
...
def _scaffold_go(
    ...
    """Scaffold a Go agent (Tier-2 polyglot). MCP servers in Go are not yet
    supported — that's a follow-up issue."""
    if not json_output:
        console.print()
        console.print(
            Panel(
                "[bold]New Go Project[/bold]\n"
                "[dim]A minimal Go agent using the AgentBreeder Go SDK.[/dim]",
                title="[bold]AgentBreeder[/bold]",
                border_style="blue",
                padding=(1, 2),
            )
    ...
    if framework_key:
        fw = next((f for f in GO_FRAMEWORKS if f["key"] == framework_key), None)
        if fw is None:
            console.print(
                f"[red]Unknown Go framework: {framework_key}. "
                f"Choose: {', '.join(f['key'] for f in GO_FRAMEWORKS)}[/red]"
            )
            raise typer.Exit(code=1)
    elif len(GO_FRAMEWORKS) == 1:
        fw = GO_FRAMEWORKS[0]
    else:
        fw = _prompt_selection("What Go framework?", GO_FRAMEWORKS)
    ...
    files = {
        "main.go": _go_main_go(name),
        "main_test.go": _go_main_test_go(name),
        "go.mod": _go_mod(name),
        "agent.yaml": _go_agent_yaml(name, fw["key"], cloud["key"], team, owner),
        "Dockerfile": _go_dockerfile(),
        ".gitignore": _go_gitignore(),
        ".env.example": _go_env_example(),
        "README.md": _go_readme(name, fw["key"], cloud["key"]),
    }
    created_files = _write_files(files, project_dir, json_output)
    ...
    console.print()
    rel_dir = os.path.relpath(project_dir, Path.cwd())
    cd_cmd = "" if rel_dir == "." else f"cd {rel_dir}\n  "
    console.print(
        Panel(
            f"[bold green]Project created![/bold green] ⚙️ {name}\n\n"
            f"  [bold]Next steps:[/bold]\n\n"
            f"  [dim]$[/dim] {cd_cmd}go mod tidy\n"
            f"  [dim]$[/dim] go run .\n"
            f"  [dim]$[/dim] go test -race -cover ./...\n"
            f"  [dim]$[/dim] agentbreeder validate agent.yaml\n"
            f"  [dim]$[/dim] agentbreeder deploy agent.yaml --target {cloud['key']}",
            title="Ready",
            border_style="green",
...
def init(
    ...
    language: str = typer.Option(
        ...
        "--language",
        "--lang",
        help="Language runtime: python | node | go",
    ),
    ...
    if language not in ("python", "node", "go"):
        console.print(f"[red]Unknown language: {language}. Choose: python, node, go[/red]")
        raise typer.Exit(code=1)
    ...
    if language == "go":
        if agent_type == "mcp-server":
            console.print(
                "[red]MCP servers in Go are not yet supported. "
                "Track I phase 2 will add Kotlin/Rust/.NET; Go MCP is a follow-up.[/red]"
            )
            raise typer.Exit(code=1)
        _scaffold_go(
